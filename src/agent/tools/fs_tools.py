...
async def create_directory_tool(path: str):
    ...
    async with fh.aopen(WORKSPACE) as session:
        ...
        print("[create_directory] args:", path, "full_path:", full_path, "res:", res)
...
async def write_file_tool(path: str, content: str):
    ...
    async with fh.aopen(WORKSPACE) as session:
        ...
        print("[write_file] args:", path, "full_path:", full_path, "len:", len(content), "res:", res)

        return f"wrote: {path} ({len(content)} bytes)"

@tool('list_directory')
async def list_directory_tool(path: str = "."):
    ...
    async with fh.aopen(WORKSPACE) as session:
        ...
        print("[list_directory] args:", path, "full_path:", full_path, "res:", res)
